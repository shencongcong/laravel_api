...
class ApiBaseController extends Controller
{
    ...
    {
        $token = JWTAuth::createToken(1, $arr);
        return $token;
    }
    ...
    * */
     ...
     * 根据token获取信息
     * */
    public static function getMesByToken()
    {
        return JWTAuth::parseToken()->getTokenData();
    }

    /*
     * 错误返回
     * */
    public static function errorResponse($status = 406, $message = "失败")
    {
        $response = array(
            'status_code' => $status,
            'message'     => $message,
     ...
     * 成功返回
     * */
    public static function successResponse($status = 200, $message = "请求成功", $data = [])
    {
        $response = array(
            'status_code' => $status,
            'message'     => $message,
    ...
    /*
     * 处理客户端请求数据
     * 如果需要展示原来的html标签时 用stripslashes()方法 处理转义后的数据
     * */
    public function dealRequest(array $attr)
    {
        foreach ($attr as $k => $v) {
            $attr[$k] = is_array($v)?$this->dealRequest($v):addslashes($this->remove_xss(trim($v)));
        }
        return $attr;
    }
    ...
    {
       return addslashes($this->remove_xss(trim($str)));
    }
