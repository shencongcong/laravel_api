...
@section('admin-content')
    <div id="content" class="content">
        ...
        <!-- end breadcrumb -->
        <!-- begin page-header -->
        <h1 class="page-header">公众号列表 <small>header small text goes here...</small></h1>
        <!-- end page-header -->
        <!-- begin row -->
        <div class="row">
            <!-- begin col-6 -->
            <div class="col-md-12">
                ...
                <div class="panel panel-inverse" data-sortable-id="table-basic-5">
                    ...
                    <div class="panel-body">
                        @if(auth('admin')->user()->can('public.add'))
                        <a href="{{ url('admin/public/create') }}">
                        ...
                        <table class="table table-bordered table-hover" id="datatable">
                            ...
                            <tbody>
                            @foreach($public['data'] as $data)
                                <tr id="{{ $data['id'] }}"  >
                                    <td>{{ $data['id'] }}</td>
                                    <td>{{ $data['public_name'] }}</td>
                                    <td>{{ $data['token'] }}</td>
                                    <td>{{ $data['wx_user_num'] }}</td>
                                    <td>{{ date("Y-m-d",$data['created_at']) }}</td>
                                    <td>{{  date("Y-m-d",$data['updated_at']) }}</td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
